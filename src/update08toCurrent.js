...
const results = [
  ...
  ["Lin Xi", "Open: Google App:Open Source", "Nationals", 2011, 1],
  [
    "Kathy Sun",
    "Gary Wan",
    "Wendy Zhao",
...
export default class UpdateDB2008 extends React.Component {
  constructor(props) {
    super(props);
  }
  ...
  updateDB() {
    ...
    for (let i = 0; i < results.length; i++) {
      let numPeople = 5 - (9 - results[i].length);
      let people = [];
      for (let k = 0; k < numPeople; k++) {
        people.push(results[i][k]);
      }

      let event = results[i][numPeople];
      let conf = results[i][numPeople + 1];
      let year = results[i][numPeople + 2];
      let place = results[i][numPeople + 3];
      for (let k = 0; k < numPeople; k++) {
  ...
  render() {
    return <Button onClick={this.updateDB}>Update</Button>;
  }
}
